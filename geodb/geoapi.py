...
def getRiskExecuteExternal(filterLock, flag, code, yy=None, mm=None, dd=None, rf_type=None, bring=None):
        ...
        if flag != 'entireAfg' or date_params:
            ...
            temp = dict([(c['agg_simplified_description'], c['count']) for c in counts])
            response['water_body_pop']=round(temp.get('Water body and Marshland', 0) or 0,0)
            response['barren_land_pop']=round(temp.get('Barren land', 0) or 0,0)
            response['built_up_pop']=round(temp.get('Build Up', 0) or 0,0)
            response['fruit_trees_pop']=round(temp.get('Fruit Trees', 0) or 0,0)
            response['irrigated_agricultural_land_pop']=round(temp.get('Irrigated Agricultural Land', 0) or 0,0)
            response['permanent_snow_pop']=round(temp.get('Snow', 0),0)
            response['rainfed_agricultural_land_pop']=round(temp.get('Rainfed', 0) or 0,0)
            response['rangeland_pop']=round(temp.get('Rangeland', 0) or 0,0)
            response['sandcover_pop']=round(temp.get('Sand Covered Areas', 0) or 0,0)
            response['vineyards_pop']=round(temp.get('Vineyards', 0) or 0,0)
            response['forest_pop']=round(temp.get('Forest & Shrub', 0) or 0,0)
            response['sand_dunes_pop']=round(temp.get('Sand Dunes', 0) or 0,0)

            temp = dict([(c['agg_simplified_description'], c['areaatrisk']) for c in counts])
            response['water_body_area']=round(temp.get('Water body and Marshland', 0) or 0/1000000,1)
            response['barren_land_area']=round(temp.get('Barren land', 0) or 0/1000000,1)
            response['built_up_area']=round(temp.get('Build Up', 0) or 0/1000000,1)
            response['fruit_trees_area']=round(temp.get('Fruit Trees', 0) or 0/1000000,1)
            response['irrigated_agricultural_land_area']=round(temp.get('Irrigated Agricultural Land', 0) or 0/1000000,1)
            response['permanent_snow_area']=round(temp.get('Snow', 0) or 0/1000000,1)
            response['rainfed_agricultural_land_area']=round(temp.get('Rainfed', 0) or 0/1000000,1)
            response['rangeland_area']=round(temp.get('Rangeland', 0) or 0/1000000,1)
            response['sandcover_area']=round(temp.get('Sand Covered Areas', 0) or 0/1000000,1)
            response['vineyards_area']=round(temp.get('Vineyards', 0) or 0/1000000,1)
            response['forest_area']=round(temp.get('Forest & Shrub', 0) or 0/1000000,1)
            response['sand_dunes_area']=round(temp.get('Sand Dunes', 0) or 0/1000000,1)
